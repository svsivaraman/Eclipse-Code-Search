...
public class RepositoryManager {
   ...
   /**
	    ...
	    try {
	    	...
	    	for(int noiseCount=0;noiseCount<searchTags.length;noiseCount++) {
	    		...
	    		for(i=0; i<tagNodeList.getLength();i++){
	    			 if(!tagList.contains(tagNodeList.item(i).getNodeValue())){
	    		...
	    		for(i=0; i<titleNodeList.getLength();i++){
	    			 if(!titleList.contains(titleNodeList.item(i).getNodeValue())){
	        	...
	        	for(i=0; i<snippetNodes.getLength();i++){
	        		if(!snippetList.contains(snippetNodes.item(i).getNodeValue())){
	    		...
	    		for(i=0; i<deleteNodes.getLength();i++){
	    			 if(!snipIDList.contains(deleteNodes.item(i).getNodeValue())){
    ...
    public int writeCode(String snippet,String tag,String title)
    {
		...
		//get number of code snippet in the repository
		nodeList = doc.getElementsByTagName("Code");
			...
			try {
				...
				child.setAttribute("ID","Code"+ (nodeList.getLength()+1));
